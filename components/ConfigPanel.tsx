...
export function ConfigPanel() {
  ...
  return (
    <div className="bg-neutral-900/80 border border-neutral-800 rounded-xl p-3.5 sm:p-4 shadow-sm flex flex-col flex-1 min-h-0 max-h-full overflow-hidden">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar space-y-3 pr-1 py-1">
        {/* Gemini API Key */}
      <div className="flex flex-col gap-2">
        ...
        <div className="relative">
          <input
            ...
            className="w-full bg-neutral-950 border border-neutral-800 focus:border-blue-500/80 focus:ring-1 focus:ring-blue-500/40 rounded-xl px-3 py-2 pr-10 text-xs text-neutral-100 placeholder:text-neutral-600 transition-colors font-mono"
            value={geminiKey}
      ...
      <div className="flex flex-col gap-2">
        ...
        <input
          type="text"
          className="bg-neutral-950 border border-neutral-800 focus:border-blue-500/80 focus:ring-1 focus:ring-blue-500/40 rounded-xl px-3 py-2 text-xs text-neutral-100 placeholder:text-neutral-600 transition-colors mt-0.5"
          value={contentTopic}
      ...
      <div className="flex flex-col gap-2">
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-xl p-3.5 sm:p-4 flex flex-col items-center justify-center text-center cursor-pointer transition-all ${
            isDragging
              ? "border-blue-500 bg-blue-500/10 text-blue-300"
              : "border-neutral-800 hover:border-neutral-700 bg-neutral-950/60 text-neutral-400"
          }`}
        >
          <div className="w-8 h-8 rounded-full bg-neutral-900 flex items-center justify-center mb-1.5 border border-neutral-800 text-neutral-300">
      ...
      {/* Bottom Action Area (Pinned at bottom of card) */}
      <div className="shrink-0 pt-2.5 border-t border-neutral-800 flex flex-col gap-2">
        {/* Primary Generate Button */}
        <button
          type="button"
          onClick={handleGenerateQuotes}
          disabled={isGeneratingQuotes || isGenerating}
          className="w-full flex items-center justify-center gap-2 bg-white text-neutral-950 hover:bg-neutral-200 disabled:opacity-50 disabled:cursor-not-allowed font-semibold py-2.5 px-3 rounded-lg shadow transition-all text-xs tracking-tight cursor-pointer"
        >
          {isGeneratingQuotes ? (
            <>
              <Loader2 className="w-3.5 h-3.5 animate-spin text-neutral-900" />
              <span className="truncate">{generationProgress || "Generating Visuals..."}</span>
            </>
          ) : (
            <>
        ...
        {isGenerating && (
          <div className="flex flex-col gap-1.5 bg-neutral-950 border border-blue-500/30 p-2.5 rounded-lg shadow-inner">
        ...
        {/* Secondary Export Actions: Render & Zip and Telegram */}
        <div className="grid grid-cols-2 gap-2">
          <button
            type="button"
            onClick={() => handleExport("zip")}
            disabled={queue.length === 0 || isGenerating || isGeneratingQuotes}
            ...
            title="Render all queue items and download as ZIP"
          >
            <DownloadCloud className="w-3.5 h-3.5" />
            <span>Render & Zip</span>
          </button>
          ...
          <button
            ...
            title="Render and send directly to Telegram"
          >
